...
<div class='container bgf'>
	...
	<main class="pl15 pr15 borBox" v-if='active < 2'>
		...
		<div class="flex justs alic lh60">
			<span>上传发票：<van-button type="primary" v-if="invoiceObj.flag == 'Y'&&detail.status!=1&&detail.status!=0" size="small" @click="showInvoice = true">已上传，选择发票</van-button></span>
		...
		<!-- 帮助说明 end -->
		<!-- 选择发票 start-->
		<van-dialog v-model="showInvoice" title="选择发票" show-cancel-button @confirm='checkedFun'>
			<div class="dialog-box">
				<van-checkbox-group v-model="checkedImg" >
					<div v-for="item in invoiceObj.invoiceList" :key="item.id" >
						<van-divider>{{item.invoiceNum}}</van-divider>
						<van-checkbox v-for="n in item.invoiceUrl.split(',')" :key="n" :name="n" shape="square" class="checkBox">							
							<div class="flex justc alic">
								<img :src="n" style="width:86%"/>
								<span class="color54 txtno ml5" @click.stop="showInvoiceImg(n)">查看</span>
							</div>
						</van-checkbox>
					</div>	
				</van-checkbox-group>
...
methods: {
	...
	submit(){ 
      ...
      this.$http.saveMedicineApi(val).then((res) => {
		...
		checkInvoice(){
			this.$http.checkInvoiceApi({patientId:this.$route.query.patientId}).then((res) => {
				if (res.data) {
					if (res.data.code == 200) {
						console.log(res.data.data)
		...
		checkedFun(){
			let newArr = []
			this.checkedImg.map((item)=>{
				if(item){
					newArr.push({url:item, isImage: true}) 
				}
			})
			this.form.invoiceOneUrl = newArr
		},
		showInvoiceImg(url){
